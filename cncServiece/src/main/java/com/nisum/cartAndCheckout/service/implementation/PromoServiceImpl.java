...
public class PromoServiceImpl implements PromoService {
    ...
    @Override
    public PromoDTO validatePromoCode(String promoCode, List<Integer> productIds) {
        try {
            ...
            // Get all promos for the products
            List<PromoDTO> promos = getPromosForProducts(productIds);
            
            // Find the promo that matches the code
            return promos.stream()
                    .filter(promo -> promoCode.equals(promo.getPromoCode()))
                    .findFirst()
                    .orElseThrow(() -> new IllegalArgumentException("Invalid promo code"));

        } catch (IllegalArgumentException e) {
            throw new PromoServiceUnavailableException("Promo validation failed.", e);
        } catch (Exception e) {
            throw new PromoServiceUnavailableException("Promo validation failed.", e);
        }
    }
